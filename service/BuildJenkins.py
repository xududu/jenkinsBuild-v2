...
class build_main_obj(object):
    ...
    def _build_job_obj_auxiliary_function(self, build_name, env_name, **kwargs):
        """_build_job_obj的辅助函数，为了减少重复代码
        :param build_name: job名称
        :param env_name: 环境名称
    ...
    def _build_job_obj(self, b_group, env_name, b_img=None, img_v=None, all_img_obj=None, **kwargs):
        """
        build Jenkins项目调用的函数,build_option调用此函数
        :param b_group: 传入需要build到的组名
        :param env_name: 传入环境名称
        ...
        if all_img_obj:
            for job in all_img_obj:
                ...
                if job['color'] == 'disabled':
                    continue
                if b_img == 'all':
                    self._build_job_obj_auxiliary_function(build_name=job['name'], env_name=env_name)
                # 如果只build某些model
            ...
            for group in self.group_list:
                ...
                # 数据库版本号更新
                group_id = operationdata.select_group_id_function(env=env_name, group_name=group)
                operationdata.update_img_ver_function(img=b_img, ver=img_v, group_id=group_id)
    ...
    def build_option(self, img_version, ms_group, env_name, **kwargs):
        """
        ...
        if img_version in self.model_list:
            ...
            self._build_job_obj(b_group=ms_group, env_name=env_name, b_img=img_version, all_img_obj=all_jobs_name_obj, **kwargs)
        else:
            ...
            for img in img_version_dict:
                ...
                self._build_job_obj(b_group=ms_group, env_name=env_name, b_img=img, img_v=version, **kwargs)
        return True

    def insert_new_jobs(self, new_jobs, group, env_name):
        """
        ...
        for img in img_jobs_dict:
            ...
            group_id = operationdata.select_group_id_function(env=env_name, group_name=group)
            operationdata.new_job_insert_function(new_job=single_img_jobs_dict, group_id=group_id)
